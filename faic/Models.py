...
class Models():
    ...
    def run_detect(self, img, detect_mode='Retinaface', max_num=1, score=0.5):
        ...
        if not self.retinaface_model:
            self.retinaface_model = onnxruntime.InferenceSession('.\models\phase1.bin', providers=self.providers)
    ...
    def run_recognize(self, img, kps):
        if not self.recognition_model:
            self.recognition_model = onnxruntime.InferenceSession('.\models\emb.bin', providers=self.providers)
    ...
    def run_swapper(self, image, embedding, output):
        if not self.swapper_model:
            ...
            self.swapper_model = onnxruntime.InferenceSession( "./models/phase2.bin", providers=self.providers)
    ...
    def run_GPEN_256(self, image, output):
        if not self.GPEN_256_model:
            self.GPEN_256_model = onnxruntime.InferenceSession( "./models/phase3.bin", providers=self.providers)

        io_binding = self.GPEN_256_model.io_binding()         
        io_binding.bind_input(name='input', device_type='cuda', device_id=0, element_type=np.float16, shape=(1,3,256,256), buffer_ptr=image.data_ptr())
        io_binding.bind_output(name='output', device_type='cuda', device_id=0, element_type=np.float16, shape=(1,3,256,256), buffer_ptr=output.data_ptr())
    ...
    def detect_retinaface(self, img, max_num, score):
        ...
        io_binding.bind_input(name='input.1', device_type='cuda', device_id=0, element_type=np.float16,  shape=det_img.size(), buffer_ptr=det_img.data_ptr())
